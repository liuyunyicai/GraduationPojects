function [CycleRoute, CycleLength, CycleMean, BestRoute ,best_length, AntArriveTimes]=ACO5(GridInfo, EntranceGrid, ExitGrids, FireGrids, cycle_max, ant_num, AcoParameters, GridParameters, OptimizeTypes, FireParameters, max_step, PreferedGrids, TempPrefers)
%%  入参：
...
%                                             end
                                            ...
                                            if (abs(fire_z - next_z) <= deta_dis_z) && (abs(fire_x - next_x) <= deta_dis) && (abs(fire_y - next_y) <= deta_dis)  % 仅考虑在同一楼层的火源影响
                                                ...
                                                t_s = (((next_x - fire_x) * interval_x) ^ 2 + ((next_y - fire_y) * interval_y) ^ 2 + ((next_z - fire_z) * interval_z * 5) ^ 2) ^ 0.5;
...
%                                       exit_thita = exit_thita * (exit_thita_param ^ (cycle_times - 1));  
                                      exit_thita = prefer_sizes / 3;
                                  end
                                  
                                  distance = getDistance([next_x, next_y, next_z], ExitGrids(k, :));
                                  if (distance < 20)
                                      exit_thita = 0.001;
                                  end
...
%                                         end
                             ...
                             prefer_z_param = 1000;
                             temp_prefer_size = size(TempPrefers, 1);
                             
%                              for k = 1 : temp_prefer_size
%                                  if (next_z == TempPrefers(k, 3)) && (next_z ~= 1)  % 只考虑同一楼层的出口位置
%                                      if (GridInfo(TempPrefers(k, 1), TempPrefers(k, 2), TempPrefers(k, 3), GRID_INDEX_ISFREE) == ISACCESSED_ZONE) 
%                                          deta_x = TempPrefers(k, 1) - next_x; 
%                                          deta_y = TempPrefers(k, 2) - next_y; 
%                                          deta_z = TempPrefers(k, 3) - next_z; 
% 
%                                          deta_l = (deta_x ^ 2 + deta_y ^ 2 + deta_z ^ 2);
%                                          if deta_l ~= 0
