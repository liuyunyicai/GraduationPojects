function [CycleRoute, CycleLength, CycleMean, BestRoute ,best_length, AntArriveTimes]=ACO5(GridInfo, EntranceGrid, ExitGrids, FireGrids, ...
    cycle_max, ant_num, AcoParameters, GridParameters, OptimizeTypes, FireParameters, max_step, PreferedGrids, SpecialNodes)
%%  入参：
...
total_dis_length = getDistance([x_max, y_max, z_max], [0, 0, 0], [interval_x, interval_y, interval_z]);
...
while cycle_times <= cycle_max
    ...
    TimesCache = zeros(ant_num,  max_step + 1);
...
%            count = 0;
   for j = 1 : ant_num                   % 对每只蚂蚁进行操作 
	   count = 0;
       CachRoute = zeros(cache_size, dimen_num);
...
%			    L(j) = i - 1;
			...
			for temp_size = 1 : nextval_size
				...
				if ((next_x >= 1) && (next_x <= x_max)) && ((next_y >= 1) && (next_y <= y_max)) && ((next_z >= 1) && (next_z <= z_max))                    %判断该对应点是否存在                                                                                                
                     ...
                     Searched(:, :) = Route(j, :, :);
%                      Searched(:, :)  = Visited_Route(:, :) ;
					 if find(ismember(Searched, ToSearch, 'rows')) > 0 
						is_visited = 1; 
                     end
                     
                     if  (find(ismember(CachRoute, ToSearch, 'rows')) > 0)
                      	is_cached = 1; 
                     end   
					 p_value = 0;
					 p_coeff = 1; % 系数
					 ...
					 if NextGridInfo(GRID_INDEX_ISFREE) ~= ISACCESSED_ZONE  % 如果是障碍物或者火灾区域，转移概率为0
                         ...
                         if (OptimizeTypes(OPTIMIZE_10) == 1) || (OptimizeTypes(OPTIMIZE_ALL) == 1) 
                             ...
                             if (find(ismember(PreferedGrids, ToSearch, 'rows')) > 0)
                                      ...
                                      if (GridInfo(ToSearch(1), ToSearch(2), ToSearch(3), GRID_INDEX_ISFREE) == ISACCESSED_ZONE) ...
                                            && ((ToSearch(3) > 1) && (GridInfo(ToSearch(1), ToSearch(2), ToSearch(3) - 1, GRID_INDEX_ISFREE) == ISACCESSED_ZONE))
                                            p_coeff = 5;
                                      else
                                          p_coeff = 1;
                                      end
                                        ...
                                        % 升温公式
                                         for firenum = 1 : firenums
                                           ...
                                           deta_dis_z = 0;
                                            ...
                                            if (abs(fire_z - next_z) <= deta_dis_z) && (abs(fire_x - next_x) <= deta_dis) && (abs(fire_y - next_y) <= deta_dis)  % 仅考虑在同一楼层的火源影响
                                                % 计算火源到用户当前位置的距离
                                                Tfire = GridInfo(fire_x, fire_y, fire_z, GRID_INDEX_TEMPERATURE);
                                                Tz =  345 * log10(8 * cur_time / 60 + 1); % 按照最坏估计更新火源的温度值
                                                t_s = (((next_x - fire_x) * interval_x) ^ 2 + ((next_y - fire_y) * interval_y) ^ 2 + ((next_z - fire_z) * interval_z) ^ 2) ^ 0.5;
...
%                                         GridInfo(next_x, next_y, next_z, GRID_INDEX_ISFREE) = FIRED_ZONE;
    ...
    %                                                 C_deta =  (warning_time / cur_time) ^ 1.5 * exp((-slength) / (4 * k_somke * cur_time));
                             ...
                             if v_cur == 0
                                 TempTimes(j, L(j) + 1, temp_size) = inf;
                             else
                         ...
                         if (OptimizeTypes(OPTIMIZE_1) == 1) || (OptimizeTypes(OPTIMIZE_ALL) == 1)
                             ...
                             prefer_sizes = size(PreferedGrids, 1);
                             cal_prefer_sizes = 0; % 楼道等出口的引力作用
                             prefer_z_param = 10;
                             prefer_thita = 1;
                             for k = 1 : prefer_sizes
%                                  is_cached = 0;
%                                  if  (find(ismember(CachRoute, PreferedGrids(k, :), 'rows')) > 0)
%                                     is_cached = 1; 
%                                 end   
                                 
                                 if (next_z == PreferedGrids(k, 3))   % 只考虑同一楼层的出口位置
                                     if ((next_z == 1))
                                         prefer_thita = -1;
                                     end
...
%                                              && ((PreferedGrids(k, 3) <= 3)|| ((PreferedGrids(k, 3) >= 3) && (((PreferedGrids(k, 3) > 1) && (GridInfo(PreferedGrids(k, 1), PreferedGrids(k, 2), PreferedGrids(k, 3) - 2, GRID_INDEX_ISFREE) == ISACCESSED_ZONE)))))
                                         
                                         TestGrids(1, :) = PreferedGrids(k, :);
                                         deta_x = PreferedGrids(k, 1) - next_x; 
                                         deta_y = PreferedGrids(k, 2) - next_y; 
                                         deta_z = PreferedGrids(k, 3) - next_z; 
                                         
                                         cal_prefer_sizes  = cal_prefer_sizes + 1;

                                         deta_l = (deta_x ^ 2 + deta_y ^ 2 + deta_z ^ 2) ^ 0.5;
                                         if (deta_l < 10)
                                             prefer_thita = 0.001;
                                         end
    ...
    %                                          end
                                             ...
                                             if PreferedGrids(k, 3) ~= 1
                                                    f_x = getFxDis(f_x,total_dis_length,  deta_x, deta_l, prefer_thita);
                                                    f_y = getFxDis(f_y,total_dis_length,  deta_y, deta_l, prefer_thita);
                                                    f_z = getFxDis(f_z,total_dis_length,  deta_z, deta_l, prefer_thita);
                                             end
                                         else
                            ...
                            specials_size =  size(SpecialNodes, 1);
                            prefer_thita = 1;
                            for k = 1 : specials_size
                                if (next_z == SpecialNodes(k, 3))   % 只考虑同一楼层的出口位置
                                     deta_x = SpecialNodes(k, 1) - next_x; 
                                     deta_y = SpecialNodes(k, 2) - next_y; 
                                     deta_z = SpecialNodes(k, 3) - next_z; 
...
%                                         end
                                 ...
                                 distance = getDistance([next_x, next_y, next_z], ExitGrids(k, :), [interval_x, interval_y, interval_z]);
                                  if (distance < 10)
                                      exit_thita = 0.001;
                                  end
...
%                                         end
                                     ...
                                     f_x = getFxDis(f_x,total_dis_length,  deta_x, deta_l, exit_thita);
                                     f_y = getFxDis(f_y,total_dis_length,  deta_y, deta_l, exit_thita);
                                     f_z = getFxDis(f_z,total_dis_length,  deta_z, deta_l, exit_thita);
...
%                 f_cigma = (f_param ^ (cycle_times - 1))  * f_cigma;
                ...
                if (OptimizeTypes(OPTIMIZE_9) == 1) || (OptimizeTypes(OPTIMIZE_ALL) == 1)
                    ...
                    for ff_i = 1 : cycle_times
                        f_cigma = f_cigma - 0.5^ (cycle_times -  1);
                    end
                end
...
%                               fprintf('line= %d, rand = %d\n',line, rand_line);
					if (line >= rand_line) || (sum_P == 0)
                        next_x = x + nextval_x;
                        next_y = y + nextval_y;
                        next_z = z + nextval_z;                

                        if ((next_x >= 1) && (next_x <= x_max)) && ((next_y >= 1) && (next_y <= y_max)) && ((next_z >= 1) && (next_z <= z_max))                    %判断该对应点是否存在 
                            if  GridInfo(next_x, next_y, next_z, GRID_INDEX_ISFREE) == ISACCESSED_ZONE % 剔除掉障碍物情况
    ...
    %                             Time(j) = Time(j) + TempTimes(j, L(j), next_size);
                                temp_time = TempTimes(j, L(j) + 1, next_size);
                                if  temp_visited > 0							
                                    temp_time =TimesCache(j, temp_visited); 
                                    for index = (temp_visited) : L(j) % 将重复值里面的路径全部清空
                                        Route(j, index, : ) = [-1, -1, -1];									
                                        Time(j) = Time(j) - TimesCache(j, index);
                                        TempTimes(j, index, next_size) = 0;
                                        TimesCache(j, index) = 0;
                                    end
                                    TempTimes(j, L(j) + 1, next_size) = 0;
                                    if temp_visited == 1
                                        L(j) = 1; 
                                    else
                                        L(j) = temp_visited - 1; 
                                ...
                                TempTimes(j, L(j) + 1, next_size) = temp_time;
                                TimesCache(j, L(j) + 1) = temp_time;
                                  % 选中转移节点后，跳出循环
                                  break;
                            end
                        end
			...
			if (L(j) <= grid_size)
				if (NextGrid_Selected(1, :) == [0, 0, 0]) | (Step(j) > max_step) %表示停滞在原地 或者超过最长步长
                    ...
                    B = [NextGrid_Selected(1, 1)  NextGrid_Selected(1, 2) NextGrid_Selected(1, 3)];
                    next_size = find(ismember(Nextvals,B,'rows'));
                    Time(j) = Time(j) + TimesCache(j, L(j));
                    
                    CachRoute(mod(Step(j) - 1, cache_size) + 1 , 1) =  x + NextGrid_Selected(1, 1);
                    CachRoute(mod(Step(j) - 1, cache_size) + 1, 2) =  y + NextGrid_Selected(1, 2);
                    CachRoute(mod(Step(j) - 1, cache_size) + 1, 3) =  z+ NextGrid_Selected(1, 3);
                    
                    if OUTPUT_ROUTE == 0
                        fprintf('Route Step%d:(%d,%d,%d), Time=%d,Temp_time=%d\n', Step(j),Route(j, L(j), 1) , Route(j, L(j), 2) , Route(j, L(j), 3), Time(j), TempTimes(j, L(j), next_size));
                    end
...
%       fprintf('j == %d, nextval = (%d, %d, %d) ',  j, AntRoute(j + 1, 1), AntRoute(j + 1, 2), AntRoute(j + 1, 3));
			if AntRoute(j + 1, :) == [-1, -1, -1] | (j == max_step -1)
                ...
                next_size = find(ismember(Nextvals,B,'rows'));
                if (next_size > 0)
                    Length(i) = Length(i) + 1/Eta(i, AntRoute(j, 1), AntRoute(j, 2), AntRoute(j, 3), next_size);   
                    Length_Time(i) = Length_Time(i) + TempTimes(i, j, next_size);   
                end
