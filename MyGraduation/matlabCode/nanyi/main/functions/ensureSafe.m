...
AllData = load('GridInfo_ensure.mat'); % 读取FDS仿真数据文件

GridInfo = AllData.GridInfo;
...
num = size(GridInfo, 1);
num1 = size(GridInfo, 2);
num2 = size(GridInfo, 3);
num3 = size(GridInfo, 4);
num4 = size(GridInfo, 5);
...
%              pre_z = round((pre_z + 1) / 2);
            T              = GridInfo( pre_x, pre_y, pre_z, GRID_INDEX_TEMPERATURE);
            Smoke  = 1 -  GridInfo(pre_x, pre_y, pre_z, GRID_INDEX_SMOKE);
            Q_CO2 = GridInfo(pre_x, pre_y, pre_z,GRID_INDEX_O2);
            Q_CO    = GridInfo( pre_x, pre_y, pre_z, GRID_INDEX_CO);
            Q_O2    = GridInfo(pre_x, pre_y, pre_z, GRID_INDEX_CO2);
...
%             z = round((z + 1) / 2);
            ...
            Smoke  = GridInfo(x, y, z, GRID_INDEX_SMOKE);
            Q_CO2 = GridInfo(x, y, z,GRID_INDEX_CO2);
            Q_CO    = GridInfo(x, y, z, GRID_INDEX_CO);
            Q_O2    = GridInfo(x, y, z, GRID_INDEX_O2);
             
             fprintf('(%d, %d, %d), Time = %d, T=%d, Smoke = %d, CO2 = %d, CO = %d, O2 = %d, people = %d\n', RouteStep(1), RouteStep(2), RouteStep(3) ...
                          , Time, T, Smoke, Q_CO2, Q_CO, Q_O2, p * 10);
             data_size = data_size + 1;
            EnsureDatas(data_size, :) = [RouteStep(1), RouteStep(2), RouteStep(3),  Time, T, Smoke, Q_CO2, Q_CO, Q_O2, p * 10, 1];
    end
end
