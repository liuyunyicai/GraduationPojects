function [CycleRoute, CycleLength, CycleMean, BestRoute ,best_length]=ACO5(GridInfo, EntranceGrid, ExitGrids, FireGrids, cycle_max, ant_num, AcoParameters, GridParameters, OptimizeTypes, FireParameters, max_step, PreferedGrids)
%%  入参：
				...
				if ((next_x >= 1) && (next_x <= x_max)) && ((next_y >= 1) && (next_y <= y_max)) && ((next_z >= 1) && (next_z <= z_max))                    %判断该对应点是否存在                                                                                                
					 ...
					 if NextGridInfo(GRID_INDEX_ISFREE) ~= ISACCESSED_ZONE  % 如果是障碍物或者火灾区域，转移概率为0
                         ...
                         if (OptimizeTypes(OPTIMIZE_FIRE) == 1) || (OptimizeTypes(OPTIMIZE_ALL) == 1) 
                             ...
                             v_cur = v0; % 最终通行的速度
                             cur_time = Time(j); % 获取当前路径上的Time时刻
                             ...
                             if (OptimizeTypes(OPTIMIZE_FIRE2) == 1) || (OptimizeTypes(OPTIMIZE_ALL) == 1)
                                if Easy_totals(1) ~= 0 % 如果为死亡区域则直接跳过剩下的计算							 
                                     ...
                                     if (C_O2 <= DEATH_O2) || (C_CO >= DEATH_CO) || (C_CO2 >= DEATH_CO2) % 超过危险浓度，限制为危险区域
                                        Easy_totals(2) = 0;
                                     else
                                         % 引入烟气的扩散公式
                                         k_somke = 0.9;
                         ...
                         if (OptimizeTypes(OPTIMIZE_1) == 1) || (OptimizeTypes(OPTIMIZE_ALL) == 1)
                             ...
                             % 获得人工势场法合力的矢量参数F
    % 							 f_x = f_x - (exit_nums + prefer_nums - fire_nums) * next_x;
    % 							 f_y = f_y - (exit_nums + prefer_nums - fire_nums) * next_y;
    % 							 f_z = f_z - (exit_nums + prefer_nums - fire_nums) * next_z;
...
%                     f_cigma = 2;
%                 elseif cycle_times > cycle_max / 4
%                     f_cigma = 3;
%                 else
%                     f_cigma = 5;  % 人工势场影响因子大小
%                 end
                f_cigma = 4;
...
%                     disp(Length(i));
   ...
   % 输出本次循环的最佳路径长度
    fprintf('第%d次循环最佳长度:%d\n', cycle_times, CycleLength(cycle_times));
   ...
   Tau=(1 - Rho) .* Tau + Delta_Tau; % 考虑信息素挥发，更新后的信息素
	...
	if (OptimizeTypes(OPTIMIZE_2) == 1) || (OptimizeTypes(OPTIMIZE_ALL) == 1)
        ...
        if L_min ~= Inf
                ...
                tau_max = 3.5;
                tau_min = 0.01;
                fprintf('Tau max=%d, min=%d\n', max(max(max(max(Tau)))), min(min(min(min(Tau)))));
