...
//            x_data = 0.5f;
        ...
        vector.x += ((detaX + X_BASE + x_data) * mTranslateScale);
        vector.y += ((detaZ + Y_BASE + y_data) * mTranslateScale * Math.sin(mRotateX));
        vector.z += ((detaZ + Y_BASE + y_data) * mTranslateScale * Math.cos(mRotateX));
        return vector;
    }
    ...
    **/
    public void drawLine(List<RouteInfo> routeInfos, String texture) {
        for (RouteInfo routeInfo : routeInfos) {
            boolean isTobeDrawed = false;
            String model_path = "";
            float rotateX = mRotateX;
            float scale = mScale;
            float index_x = routeInfo.getGrid_x();
            float index_y = routeInfo.getGrid_y();


            if (routeInfo.getGrid_z() == (mFloorNum * 2 -1)) {
                isTobeDrawed = true;
                model_path = "route.3DS";

            } else if ((routeInfo.getGrid_z() == (mFloorNum * 2 - 2)) && (texture.equals(blue_texture))) {
                isTobeDrawed = true;
                model_path = "downstairs1.3DS";
                rotateX = -45;

                StairsInfo stairsInfo = getCurStairs(routeInfo);
                if (null != stairsInfo) {
                    index_x = stairsInfo.getGrid_x();
                    index_y = stairsInfo.getGrid_y();
                }

            }

            if (isTobeDrawed) {
                Object3D cube = loadModel(model_path, scale);
                cube.strip();
                cube.build();
                cube.calcTextureWrapSpherical();
                cube.setTexture(texture);
//            cube.setCenter(mOriginVector);
                SimpleVector cubeCenter = getPoint(index_x, 0, (float)(index_y));
...
//                LogUtils.i("Floor" +mFloorNum + ":(" + cubeCenter.x + ", " + cubeCenter.y + ", " + cubeCenter.z + ")");
    ...
    private StairsInfo getCurStairs(RouteInfo routeInfo) {
        ...
        if ((null != routeInfo) && (null != stairsInfos)) {
            int index_x = routeInfo.getGrid_x();
            int index_y = routeInfo.getGrid_y();
            int index_z = routeInfo.getGrid_z();

            for (StairsInfo stairInfo : stairsInfos) {
                if ((isInMiddle(index_x, stairInfo.getX_min(), stairInfo.getX_max()))
                        && isInMiddle(index_y, stairInfo.getY_min(), stairInfo.getY_max())
                        && isInMiddle(index_z, stairInfo.getZ_min(), stairInfo.getZ_max())) {
                    return stairInfo;
                }
    ...
    * 判断该数值是否在该区间范围内
    **/
    private boolean isInMiddle(int value, int min, int max) {
